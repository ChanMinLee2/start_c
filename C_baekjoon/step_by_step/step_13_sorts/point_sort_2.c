// 1차 시도 성공 : point sort랑 x y 우선순위만 바뀜. 
#include <stdio.h>
...
int main()
{
    ...
    merge_sort_y(x_arr, y_arr, 0, count-1);
    int count_of_x = 0;
    int y_buf = 0;
    for (int i = 0; i < count; i++) {
        if(count_of_x == 0)
        ...
        else if(y_buf == y_arr[i])
        {
            // 이 케이스 문제 
            if(i == count-1)
            {
                merge_sort_x(x_arr, y_arr, i - count_of_x, i);
                break;
            }
            count_of_x++;
        }
        ...
        {
            if(count_of_x != 1)
                merge_sort_x(x_arr, y_arr, i - count_of_x, i-1);
            count_of_x = 1;
            y_buf = y_arr[i];
        }
    }
